...
from mdlinkcheck.extractor import (
    ...
    extract_links_for_files,
)
from mdlinkcheck.models import Link, LinkKind, Problem, WriteReport
...
def test_extracts_url_link() -> None:
    links = list(extract_markdown_links("[text](https://example.com)", "source.md", Path(".")))
...
def test_extracts_relative_file_link() -> None:
    links = list(extract_markdown_links("[text](next.md)", "src/source.md", Path("src")))
...
def test_collect_files(tmp_path: Path) -> None:
    ...
    assert {p.name: p for p in files} == {"a.md": tmp_path / "docs" / "a.md", "b.markdown": tmp_path / "b.markdown"}
...
def test_http_link_returns_unresolved(tmp_path: Path) -> None:
    ...
    assert any(report.get("message") == "HTTP checking is not supported in offline mode" for report in reports)
...
def test_text_report_with_problems() -> None:
    ...
    reports = [{"type": "file", "raw": "doc.md", "ok": False, "message": problem.message}]
    report = write_text_report([problem], reports)
    assert "Problems: 1" in report.text
    assert "doc.md" in report.text


def test_cli_missing_path_exits_with_error(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
